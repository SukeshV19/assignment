  ...
  return (
    <div className="m-1 w-full flex items-start justify-center">
      <div className='flex flex-col items-start m-2 p-2 justify-start w-[400px] shadow-md bg-gray-950 h-[90vh]'>
        ...
        {messages.map((msg, index) => {
          ...
          return (
            <div key={index} className="flex flex-col items-end justify-end w-full">
              <p
                className={`
          bg-blue-500 text-white mt-[2px]
          ${isFirst ? 'p-2 rounded-t-xl rounded-l-xl' : ''}
          ${isLast ? 'p-2 rounded-b-xl rounded-l-xl' : ''}
          ${isMiddle ? 'p-1 rounded-sm' : ''}
        `}
              >
                {msg}
              </p>
        ...
        <div className="w-full mt-3 flex items-center px-3 gap-2">
          {/* Camera icon */}
          <MdPhotoCamera className="text-white bg-blue-500 p-[5px] rounded-full text-3xl" />

          {/* Input field */}
          <input
            className="flex-1 rounded-full bg-zinc-700 text-gray-100 p-3"
            placeholder="Message..."
            value={msg}
          ...
          <TbPhotoVideo className="text-white text-2xl" />
          <LuSmilePlus className="text-white text-2xl" />
          ...
          >
            <IoMdSend className="text-blue-400 ml-1 text-2xl" />
          </button>
        </div>
      </div>
    </div>
